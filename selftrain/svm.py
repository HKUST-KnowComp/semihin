...
X = vstack((vectors_train,vectors_test)).todense()
...
for r in range(10):
  ...
  ys = -np.ones(len(y)) # -1 denotes unlabeled point
  random_labeled_points = random.sample(np.where(y == 0)[0], labeled_N/3)+\
  random.sample(np.where(y == 1)[0], labeled_N/3) + \
  random.sample(np.where(y == 2)[0], labeled_N/3)
  #+ random.sample(np.where(newsgroups_train.target == 2)[0], labeled_N/3)

  ys[random_labeled_points] = y[random_labeled_points]
  # train
  #svc = LR()
  #svc = SVC(probability=True)
  #clf = SelfLearningModel(svc)
  clf = scikitTSVM.SKTSVM()
  #clf.fit(vectors_train[random_labeled_points], ys)
  clf.fit(X,ys)

  # test
  pred = clf.predict(vectors_test.todense())
  print metrics.precision_score(newsgroups_test.target, pred, average='weighted')
